...
public class SocialLogin extends Controller {
	...
	public static Result logout() {
		...
		return redirect(routes.SocialLogin.info());
	}
	...
	public static Result authenticateDone(String provider) {
		...
		try {
			...
			SocialAuthService userService = SocialAuthService.getInstance();
			if (log.isDebugEnabled())
				log.debug("userService : " + userService);
			
			final SocialUser user = userService.createNew(profile);
			if (log.isDebugEnabled())
				log.debug("user : " + user);
		...
		return redirect(controllers.routes.SocialLogin.info());
	}
